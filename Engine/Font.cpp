...
void Font::DrawText( const std::string& text,const Vei2& pos,Color color,Graphics& gfx ) const
{
	...
	for( auto c : text )
	{
		...
		else if( c >= firstChar + 1 && c <= lastChar )
		{
			...
			gfx.DrawSprite( curPos.x,curPos.y,
				MapGlyphRect( c ),surf,false,e );
		}
...
void Font::DrawTextCentered( const std::string& text,
	const Vei2& center,Color color,Graphics& gfx ) const
{
	const auto textWidth = GetGlyphSize().x * int( text.length() );
